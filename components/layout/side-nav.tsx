...
export function SideNav() {
  ...
  return (
    <aside className="hidden w-56 shrink-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:flex md:flex-col">
      <div className="mb-6">
        <p className="text-xs font-semibold uppercase text-slate-400">Reminder App</p>
        <h1 className="mt-2 text-lg font-semibold text-balance text-slate-950">到期提醒</h1>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              className={cn(
                "group flex items-center gap-2.5 rounded-lg px-3 py-2 font-medium transition-colors",
                isActive
