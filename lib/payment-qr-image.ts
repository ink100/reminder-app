...
export async function normalizePaymentQrImage(input: Buffer) {
  try {
    ...
    if ((metadata.pages ?? 1) > 1) throw new Error("不支持动态或多帧二维码图片");

    // Fully decode and re-encode as PNG. This strips trailing/polyglot payloads and
    // ensures the object served from R2 matches its Content-Type and extension.
    ...
    if (error instanceof Error && (
      ...
      error.message === "不支持动态或多帧二维码图片" ||
      error.message === "仅支持 PNG、JPG、WebP、GIF 图片"
    )) {
      throw error;
    }
    throw new Error("无法解析二维码图片，请上传有效的 PNG、JPG、WebP 或 GIF 图片");
  }
}
