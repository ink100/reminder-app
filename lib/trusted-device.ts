...
export async function createTrustedDevice(ipAddress?: string | null, userAgent?: string | null) {
...
export async function revokeTrustedDevice(id: string) {
  ...
  if (currentToken && hashTrustedDeviceToken(currentToken) === device.tokenHash) {
    cookieStore.delete(TRUSTED_DEVICE_COOKIE_NAME);
  }
