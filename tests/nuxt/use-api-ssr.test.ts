...
describe("useApi SSR request context", () => {
  it("uses Nuxt request-aware fetch during SSR so protected API calls forward the incoming session cookie", () => {
    ...
    expect(source).toContain('if (import.meta.server) return await useRequestFetch()<T>(url, requestOptions)');
    expect(source).toContain("return await $fetch<T>(url, requestOptions)");
  });
});
